...
{
    [PacketHeader("$EnablePortal", Authority = AuthorityType.GameMaster)]
    public class EnablePortalPacket
    {
        #region Methods
        ...
        public static void HandlePacket(object session, string packet)
        {
            ...
            {
                EnablePortalPacket packetDefinition = new EnablePortalPacket();
                packetDefinition.ExecuteHandler(sess);
            }
        }

        public static void Register() => PacketFacility.AddHandler(typeof(EnablePortalPacket), HandlePacket);

        public static string ReturnHelp() => "$EnablePortal";
            ...
            if (session.HasCurrentMapInstance)
            {
                ...
                if (portal != null)
                {
                    Logger.LogUserEvent("GMCOMMAND", session.GenerateIdentity(),
                        $"[EnablePortal]MapId: {portal.SourceMapId} MapX: {portal.SourceX} MapY: {portal.SourceY}");
                    session.SendPacket(session.Character.GenerateSay(
                        string.Format(Language.Instance.GetMessageFromKey("NEAREST_PORTAL_ENABLE"), portal.SourceMapId,
