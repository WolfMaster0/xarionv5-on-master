...
namespace OpenNos.GameObject
{
    public class ItemInstance : ItemInstanceDTO
    {
        ...
        public ItemInstance(ItemInstanceDTO input)
        {
            ...
            XP = input.XP;
            ItemOptions = input.ItemOptions;
            Agility = input.Agility;
            FirstPartnerSkill = input.FirstPartnerSkill;
            SecondPartnerSkill = input.SecondPartnerSkill;
            ThirdPartnerSkill = input.ThirdPartnerSkill;
            FirstPartnerSkillRank = input.FirstPartnerSkillRank;
            SecondPartnerSkillRank = input.SecondPartnerSkillRank;
            ThirdPartnerSkillRank = input.ThirdPartnerSkillRank;
            MinimumLevel = input.MinimumLevel;
            BaseMinDamage = input.BaseMinDamage;
            BaseMaxDamage = input.BaseMaxDamage;
            BaseConcentrate = input.BaseConcentrate;
            BaseHitRate = input.BaseHitRate;
            BaseDefenceDodge = input.BaseDefenceDodge;
            BaseDistanceDefence = input.BaseDistanceDefence;
            BaseDistanceDefenceDodge = input.BaseDistanceDefenceDodge;
            BaseMagicDefence = input.BaseMagicDefence;
            BaseCloseDefence = input.BaseCloseDefence;
        }
        ...
        public ItemInstance HardItemCopy() => new ItemInstance
        {
            ...
            XP = XP,
            Agility = Agility,
            FirstPartnerSkill = FirstPartnerSkill,
            SecondPartnerSkill = SecondPartnerSkill,
            ThirdPartnerSkill = ThirdPartnerSkill,
            FirstPartnerSkillRank = FirstPartnerSkillRank,
            SecondPartnerSkillRank = SecondPartnerSkillRank,
            ThirdPartnerSkillRank = ThirdPartnerSkillRank,
            MinimumLevel = Item.LevelMinimum,
            BaseMinDamage = Item.DamageMinimum,
            BaseMaxDamage = Item.DamageMaximum,
            BaseConcentrate = BaseConcentrate,
            BaseHitRate = Item.HitRate,
            BaseDefenceDodge = Item.DefenceDodge,
            BaseDistanceDefence = Item.DistanceDefence,
            BaseDistanceDefenceDodge = Item.DistanceDefenceDodge,
            BaseMagicDefence = Item.MagicDefence,
            BaseCloseDefence = Item.CloseDefence
        };
        ...
        {
            var partnerSkillList = ServerManager.Instance.PartnerSkills.FirstOrDefault(s => s.PartnerVnum == ItemVNum);
            var strSkills =
                $"{(partnerSkillList != null && FirstPartnerSkill > 0 ? partnerSkillList.FirstSkill : 0)}.{(byte)FirstPartnerSkillRank} " +
                $"{(partnerSkillList != null && SecondPartnerSkill > 0 ? partnerSkillList.SecondSkill : 0)}.{(byte)SecondPartnerSkillRank} " +
                $"{(partnerSkillList != null && ThirdPartnerSkill > 0 ? partnerSkillList.ThirdSkill : 0)}.{(byte)ThirdPartnerSkillRank}";

            return $"pslinfo {Item.VNum} {Item.Element} {Item.ElementRate} {Item.LevelJobMinimum} {Item.Speed} {Item.FireResistance} {Item.WaterResistance} {Item.LightResistance} {Item.DarkResistance} {strSkills}";
        }

        private int GetShellWeaponEffectValue(ShellWeaponEffectType effectType)
        {
            if (Session == null)
            {
                return 0;
        ...
        public string GenerateSlInfo(bool callFormBazaar = false)
        {
            ...
            if (Session != null && !callFormBazaar)
            {
                shellHit =
                ...
                shellElement = GetShellWeaponEffectValue(ShellWeaponEffectType.SlElement)
                            + GetShellWeaponEffectValue(ShellWeaponEffectType.SlGlobal);

                shellHp = +GetShellWeaponEffectValue(ShellWeaponEffectType.Slhp)
                           + GetShellWeaponEffectValue(ShellWeaponEffectType.SlGlobal);
            }
        ...
        public void UpgradeSp(ClientSession sesion, UpgradeProtection protect)
        {
            ...
            {
                if (ServerManager.RandomNumber() < destroy[Upgrade])
                {
                    sesion.CurrentMapInstance.Broadcast(StaticPacketHelper.GenerateEff(UserType.Player, sesion.Character.CharacterId,
                    3004), sesion.Character.MapX, sesion.Character.MapY);
                ...
                else if (ServerManager.RandomNumber() < upfail[Upgrade])
                {
                    sesion.CurrentMapInstance.Broadcast(StaticPacketHelper.GenerateEff(UserType.Player, sesion.Character.CharacterId,
                    3004), sesion.Character.MapX, sesion.Character.MapY);
                    sesion.SendPacket(sesion.Character.GenerateSay(Language.Instance.GetMessageFromKey("UPGRADESP_FAILED_SAVED"), 11));
                ...
                else
                {
                    ...
                    if (ItemVNum == 907)
                    {
                        sesion.Character.Inventory.RemoveItemAmount(5107);
                    }
                    else if(ItemVNum == 900)
                    {
                        sesion.Character.Inventory.RemoveItemAmount(5207);
                    }
                    else if(ItemVNum == 4099)
                    {
                        sesion.Character.Inventory.RemoveItemAmount(5519);
                    }
                    else
                    {
                        return;
                    ...
                    sesion.CurrentMapInstance.Broadcast(StaticPacketHelper.GenerateEff(UserType.Player, sesion.Character.CharacterId,
                    3005), sesion.Character.MapX, sesion.Character.MapY);
                    ...
                    Upgrade++;
                    sesion.SendPacket(GenerateInventoryAdd());
                }
            ...
            else
            {
                ...
                if (Upgrade > 8)
                {
                    sesion.Character.Family?.InsertFamilyLog(FamilyLogType.ItemUpgraded, sesion.Character.Name, itemVNum: ItemVNum, upgrade: Upgrade);
                }
                sesion.SendPacket(GenerateInventoryAdd());
            }
            sesion.SendPacket(sesion.Character.GenerateGold());
        ...
        public string GenerateEInfo()
        {
            ...
            DateTime test = ItemDeleteTime ?? DateTime.UtcNow;
            long time = ItemDeleteTime != null ? (long)test.Subtract(DateTime.UtcNow).TotalSeconds : 0;
            ...
            switch (itemType)
            {
                ...
                case ItemType.Box:
                    switch (subtype)
                    {
                        ...
                        case 6:
                            var itemElement = ServerManager.GetItem(HoldingVNum)?.Element;
                            var strList = $"{FirstPartnerSkill} {(byte)FirstPartnerSkillRank} " +
                                $"{(byte)SecondPartnerSkill} {(byte)SecondPartnerSkillRank} " +
                                $"{ThirdPartnerSkill} {ThirdPartnerSkillRank}";
                            return HoldingVNum != 0 ? $"e_info 13 {ItemVNum} 1 {HoldingVNum} {itemElement} {strList}" : $"e_info {ItemVNum} 0";
        ...
        public void OptionItem(ClientSession session, short cellonVNum)
        {
            ...
            if (session.Character.Inventory.CountItem(cellonVNum) > 0 && Item.MaxCellon > CellonOptions.Count)
            {
                ...
                if (Item.MaxCellonLvl > dataIndex && session.Character.Gold >= goldAmount)
                {
                    ...
                    GameLogger.Instance.LogItemOption(ServerManager.Instance.ChannelId, session.Character.Name,
                        session.Character.CharacterId, this, value);
        ...
        public void RarifyItem(ClientSession session, RarifyMode mode, RarifyProtection protection, bool isCommand = false, sbyte forceRare = 0)
        {
            ...
            void Rarify(sbyte rarity)
            {
                ...
                if (mode != RarifyMode.Drop)
                {
                    if (session != null)
                    {
                        GameLogger.Instance.LogItemRarify(ServerManager.Instance.ChannelId, session.Character.Name,
                            session.Character.CharacterId, this, 1);

                        session.SendPacket(session.Character.GenerateSay(
                            string.Format(Language.Instance.GetMessageFromKey("RARIFY_SUCCESS"), Rare), 12));
                        session.SendPacket(UserInterfaceHelper.GenerateMsg(
                            string.Format(Language.Instance.GetMessageFromKey("RARIFY_SUCCESS"), Rare), 0));
                        session.CurrentMapInstance?.Broadcast(
                            StaticPacketHelper.GenerateEff(UserType.Player, CharacterId, 3005),
                            session.Character.PositionX, session.Character.PositionY);
                        session.SendPacket("shop_end 1");
                    }
                }
                bool isPartner = (ItemVNum >= 990 && ItemVNum <= 992) || (ItemVNum >= 995 && ItemVNum <= 997);
                SetRarityPoint(isPartner);
            }
            ...
            if (rnd < rare[10] && protection == RarifyProtection.Scroll && !(protection == RarifyProtection.Scroll && Rare >= 8))
            {
                Rarify(8);
            }
            ...
            else if (mode != RarifyMode.Drop && session != null)
            {
                ...
                GameLogger.Instance.LogItemRarify(ServerManager.Instance.ChannelId, session.Character.Name,
                    session.Character.CharacterId, this, 0);
        ...
        public void SetRarityPoint(bool partner)
        {
            switch (Item.EquipmentSlot)
            {
                ...
                case EquipmentType.SecondaryWeapon:
                    {
                        int point = CharacterHelper.RarityPoint(Rare, Item.IsHeroic ? (short)(95 + Item.LevelMinimum) : (partner ? MinimumLevel : Item.LevelMinimum));
                        Concentrate = (short)(partner ? BaseConcentrate : 0);
                        HitRate = (short)(partner ? BaseHitRate : 0);
                        DamageMinimum = (short)(partner ? BaseMinDamage : 0);
                        DamageMaximum = (short)(partner ? BaseMaxDamage : 0);
                        if (Rare >= 0)
                        {
                            for (int i = 0; i < point; i++)
                            {
                                int rndn = ServerManager.RandomNumber<int>(0, 3);
                                if (rndn == 0)
                                {
                                    Concentrate++;
                                    HitRate++;
                ...
                case EquipmentType.Armor:
                    {
                        ...
                        DefenceDodge = (short)(partner ? BaseDefenceDodge : 0);
                        DistanceDefenceDodge = (short)(partner ? BaseDistanceDefenceDodge : 0);
                        DistanceDefence = (short)(partner ? BaseDistanceDefence : 0);
                        MagicDefence = (short)(partner ? BaseMagicDefence : 0);
                        CloseDefence = (short)(partner ? BaseCloseDefence : 0);
                        if (Rare >= 0)
                        {
                            for (int i = 0; i < point; i++)
                            {
            ...
            if (Upgrade < 6)
            {
                ...
                if (Upgrade + itemToSum.Upgrade < 6 && ((itemToSum.Item.EquipmentSlot == EquipmentType.Gloves && Item.EquipmentSlot == EquipmentType.Gloves) || (Item.EquipmentSlot == EquipmentType.Boots && itemToSum.Item.EquipmentSlot == EquipmentType.Boots)))
                {
                    ...
                    if (rnd < upsuccess[Upgrade + itemToSum.Upgrade])
                    {
                        ...
                        GameLogger.Instance.LogItemSum(ServerManager.Instance.ChannelId, session.Character.Name,
                            session.Character.CharacterId, this, 1);
        ...
        public void UpgradeItem(ClientSession session, UpgradeMode mode, UpgradeProtection protection, bool isCommand = false)
        {
            ...
            if (Upgrade < 10)
            {
                ...
                if (Rare == 8)
                {
                    ...
                    {
                        GameLogger.Instance.LogItemUpgrade(ServerManager.Instance.ChannelId, session.Character.Name,
                            session.Character.CharacterId, this, protection, -1);
                    ...
                    {
                        GameLogger.Instance.LogItemUpgrade(ServerManager.Instance.ChannelId, session.Character.Name,
                            session.Character.CharacterId, this, protection, 0);
                        session.CurrentMapInstance.Broadcast(StaticPacketHelper.GenerateEff(UserType.Player, session.Character.CharacterId, 3004), session.Character.MapX, session.Character.MapY);
                        IsFixed = true;
                    ...
                    else
                    {
                        ...
                        if (Upgrade > 4)
                        {
                            session.Character.Family?.InsertFamilyLog(FamilyLogType.ItemUpgraded, session.Character.Name, itemVNum: ItemVNum, upgrade: Upgrade);
                        }
                        session.SendPacket(GenerateInventoryAdd());
                    }
                }
