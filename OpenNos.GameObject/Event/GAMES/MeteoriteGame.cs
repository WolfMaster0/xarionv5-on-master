    ...
    public static class MeteoriteGame
    {
        ...
        public static void GenerateMeteoriteGame()
        {
            ...
            Observable.Timer(TimeSpan.FromSeconds(290)).Subscribe(observer =>
            {
                ...
                ServerManager.Instance.Broadcast(UserInterfaceHelper.GenerateMsg(
                    string.Format(Language.Instance.GetMessageFromKey("METEORITE_SECONDS"), 10), 1));
            });
            Observable.Timer(TimeSpan.FromMinutes(5)).Subscribe(observer =>
            {
                ServerManager.Instance.Broadcast(
                    UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("METEORITE_STARTED"), 1));
            ...
            Observable.Timer(TimeSpan.FromSeconds(330)).Subscribe(observer =>
            {
                ...
                MapInstance map =
                    ServerManager.GenerateMapInstance(2004, MapInstanceType.EventGameInstance, new InstanceBag());
                if (map != null)
                {
        ...
        private class MeteoriteGameThread
        {
            #region Members
            ...
            public void Run(MapInstance map)
            {
                ...
                foreach (ClientSession session in _map.Sessions)
                {
                    ...
                    session.Character.LastSpeedChange = DateTime.UtcNow;
                    session.CurrentMapInstance?.Broadcast(session.Character.GenerateCMode());
                }

                int i = 0;

                Observable.Timer(TimeSpan.Zero).Subscribe(observer => RunRound(i++));
                Observable.While(() => _map?.Sessions?.Any() == true, Observable.Timer(TimeSpan.FromSeconds(100)))
                    .Subscribe(observer => RunRound(i++));
