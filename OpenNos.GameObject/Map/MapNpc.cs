...
namespace OpenNos.GameObject
{
    public class MapNpc : MapNpcDTO
    {
        ...
        {
            double time = (DateTime.UtcNow - LastEffect).TotalMilliseconds;
            if (time > EffectDelay)
            {
            ...
            time = (DateTime.UtcNow - LastMove).TotalMilliseconds;
            if (IsMoving && Npc.Speed > 0 && time > _movetime)
            {
                ...
                if (MapInstance.Map.GetFreePosition(ref mapX, ref mapY, xpoint, ypoint))
                {
                    ...
                    LastMove = DateTime.UtcNow.AddSeconds(value);
                    MapInstance.Broadcast(new BroadcastPacket(null, StaticPacketHelper.Move(UserType.Npc, MapNpcId, MapX, MapY, Npc.Speed), ReceiverType.All, xCoordinate: mapX, yCoordinate: mapY));
                }
            }
            if (Target == -1)
            {
            ...
            else if (Target != -1)
            {
                ...
                {
                    npcMonsterSkill = Npc.Skills.Where(s => (DateTime.UtcNow - s.LastSkillUse).TotalMilliseconds >= 100 * s.Skill.Cooldown).OrderBy(rnd => _random.Next()).FirstOrDefault();
                }
                int hitmode = 0;
                ...
                {
                    if (((DateTime.UtcNow - LastEffect).TotalMilliseconds >= 1000 + (Npc.BasicCooldown * 200) && Npc.Skills.Count == 0) || npcMonsterSkill != null)
                    {
                ...
                else
                {
                    if (IsMoving)
                    {
                        ...
                        if (DateTime.UtcNow > LastMove && Npc.Speed > 0 && Path.Count > 0)
                        {
...
#pragma warning disable RCS1213 // Remove unused member declaration.
        ...
        private void RegisterNpcLife() => Observable.Interval(TimeSpan.FromMilliseconds(400)).Subscribe(observer => StartLife());
