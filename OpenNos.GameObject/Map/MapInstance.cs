...
namespace OpenNos.GameObject
{
    public class MapInstance : BroadcastableBase
    {
        ...
        private IDisposable _instanceLife;
        ...
        public bool IsSleeping
        {
            get
            {
                if (_isSleepingRequest && !_isSleeping && LastUnregister.AddSeconds(30) < DateTime.UtcNow)
                {
                    _isSleeping = true;
        ...
        public DateTime LastQuestSpawn { get; internal set; }
        ...
        public void DropItems(List<Tuple<short, int, short, short>> list)
        {
            foreach (Tuple<short, int, short, short> drop in list)
            {
                ...
                DroppedList[droppedItem.TransportId] = droppedItem;
                Broadcast($"drop {droppedItem.ItemVNum} {droppedItem.TransportId} {droppedItem.PositionX} {droppedItem.PositionY} {(droppedItem.GoldAmount > 1 ? droppedItem.GoldAmount : droppedItem.Amount)} 0 -1");
            }
        }
        ...
        public List<string> GetMapItems()
        {
            ...
            Sessions.Where(s => s.Character?.InvisibleGm == false && s.Character?.IsVehicled == false).ToList().ForEach(s => s.Character.Mates.Where(m => m.IsTeamMember).ToList().ForEach(m => packets.Add(m.GenerateIn())));
        ...
        public void RemoveMapItem()
        {
            ...
            try
            {
                List<MapItem> dropsToRemove = DroppedList.Where(dl => dl.CreatedDate.AddMinutes(3) < DateTime.UtcNow);
        ...
        {
            _instanceLife = Observable.Interval(TimeSpan.FromSeconds(5)).Subscribe(x =>
            {
                if (InstanceBag?.EndState != 1)
                {
                    Parallel.ForEach(WaveEvents, waveEvent =>
                    {
                        if (waveEvent.LastStart.AddSeconds(waveEvent.Delay) <= DateTime.UtcNow)
                        {
                            ...
                            waveEvent.LastStart = DateTime.UtcNow;
                        }
                    });
                    try
                    {
        ...
        internal ConcurrentBag<int> SummonMonsters(List<MonsterToSummon> summonParameters)
        {
            ...
            {
                NpcMonster npcmonster = ServerManager.GetNpcMonster(npcMonster.VNum);
                if (npcmonster != null)
                {
                    MapMonster mapMonster = new MapMonster
                    {
                        MonsterVNum = npcmonster.NpcMonsterVNum,
                        MapY = npcMonster.SpawnCell.Y,
                        MapX = npcMonster.SpawnCell.X,
                        MapId = Map.MapId,
                        IsMoving = npcMonster.IsMoving,
                        MapMonsterId = GetNextMonsterId(),
                        ShouldRespawn = false,
                        Target = npcMonster.Target,
                        OnDeathEvents = npcMonster.DeathEvents,
                        OnNoticeEvents = npcMonster.NoticingEvents,
                        IsTarget = npcMonster.IsTarget,
                        IsBonus = npcMonster.IsBonus,
                        IsBoss = npcMonster.IsBoss,
        ...
        {
            NpcMonster npcMonster = ServerManager.GetNpcMonster(summonParameters.VNum);
            if (npcMonster != null)
            {
                MapNpc mapNpc = new MapNpc
                {
                    ...
                    MapNpcId = DAOFactory.MapNpcDAO.GetNextMapNpcId(),
        ...
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Usage", "CA2213:DisposableFieldsShouldBeDisposed", MessageId = "<DroppedList>k__BackingField")]
