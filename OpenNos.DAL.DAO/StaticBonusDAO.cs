    ...
    public class StaticBonusDAO : IStaticBonusDAO
    {
        ...
        public IEnumerable<StaticBonusDTO> LoadByCharacterId(long characterId)
        {
            using (OpenNosContext context = DataAccessHelper.CreateContext())
            {
                ...
                foreach (StaticBonus entity in context.StaticBonus.AsNoTracking().Where(i => i.CharacterId == characterId && i.DateEnd > DateTime.UtcNow))
                {
        ...
        public static StaticBonusDTO LoadById(long sbId)
        {
            try
            {
                ...
                {
                    StaticBonusDTO dto = new StaticBonusDTO();
                    if (Mapper.Mappers.StaticBonusMapper.ToStaticBonusDTO(context.StaticBonus.AsNoTracking().FirstOrDefault(s => s.StaticBonusId.Equals(sbId)), dto))
                    {
                        return dto;
                    }
        ...
        public IEnumerable<short> LoadTypeByCharacterId(long characterId)
        {
            try
            {
                using (OpenNosContext context = DataAccessHelper.CreateContext())
                {
                    return context.StaticBonus.AsNoTracking().Where(i => i.CharacterId == characterId).Select(qle => (short)qle.StaticBonusType).ToList();
                }
            }
            catch (Exception e)
            {
