...
#pragma warning restore IDE0044, RCS1169, 649
        ...
        public static void Main(string[] args)
        {
            try
            {
                ...
                Console.Title = $"OpenNos Game Log Server{(_isDebug ? " Development Environment" : string.Empty)}";
                ...
                if (!ignoreStartupMessages)
                {
                    ...
                    string text =
                        $"GAME LOG SERVER v{fileVersionInfo.ProductVersion}dev - PORT : {port} by OpenNos Team";
                ...
                try
                {
                    ...
                    string ipAddress = ConfigurationManager.AppSettings["GameLogIP"];
                    IScsServiceApplication server =
                        ScsServiceBuilder.CreateService(new ScsTcpEndPoint(ipAddress, port));

                    server.AddService<IGameLogService, GameLogService>(new GameLogService());
                    if (GameLogManager.Instance.AuthentificatedClients.Count != 0)
                    {
                        // Do nothing, just verify that GameLogManager is initialized before anyone can connect
                    }
